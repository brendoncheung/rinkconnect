...
[ADR 0006](0006-fees-computed-not-collected.md)'s exclusion of native apps.
...
ADR 0004's decision is otherwise unchanged: the balance reminder still ships
as email, with SMS still the named upgrade path for *that* reminder
specifically, not this one.

This ADR settles the **channel**. What triggers each reminder and how its
timing is set is settled separately in
